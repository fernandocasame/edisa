    ...
    export default {
        ...
        methods: {
            ...
            guardarCodigos(){
                ...
                var codigosEnviar = [];
                for( var i=this.porcentaje; i<this.cantidad; i++ ){
                ...
                .then(res => {
                    ...
                    alert('this.porcentaje: ' + this.porcentaje);
                    if( this.porcentaje < this.cantidad ){
                        /*condicion para validar codigos repetidos*/
                        while( this.porcentaje < this.cantidad ){
                            alert('dentro while'+this.porcentaje);
                            codigosEnviar = [];
                            this.generarCodigos(this.porcentaje, (this.cantidad+1));
                            for( var i=(this.porcentaje+1); i<=this.cantidad; i++ ){
                                alert('codigosEnviar: '+this.codigos[i].codigo);
                                codigosEnviar.push(this.codigos[i].codigo);
                            }
                            alert('codigosEnviarFORM: '+codigosEnviar);
                            var formData = new FormData();
                                formData.append("codigo", codigosEnviar);
                                formData.append("serie", this.nombre_serie);
                                formData.append("libro", this.nombre);
                            ...
                            axios.post('http://127.0.0.1:8000/api/codigosLibros', formData)
                            .then(res => {
                                alert('dentro then while res.data: '+res.data);
                                ...
                                .catch(function (error) {
                                    alert('Error dentro de while: '+error);
                                });
                ...
                .catch(function (error) {
                    alert('Error primera consulta: ' + error);
                });
